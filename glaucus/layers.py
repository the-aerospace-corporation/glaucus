...
class RMSNormalizeIQ(L.LightningModule):
    """
    ...
    def __init__(self, spatial_size: int = 4096, bias: bool = False) -> None:
        super().__init__()
        self.bias = bias
        self.scale = 1 / np.sqrt(spatial_size)
        if bias:
...
class TimeDomain2FreqDomain(L.LightningModule):
    """
    Convert Time Domain RF to Freq Domain RF.
    Expects NCL or NSCL format (batchsize, 2, spatial_size) or (batchsize, seq, 2, spatial_size).
    """
...
class FreqDomain2TimeDomain(L.LightningModule):
    """
...
class DropConnect(L.LightningModule):
    """
