...
class TestParams(unittest.TestCase):
    ...
    def test_spatial_io_glaucus(self):
        ...
        for spatial_dim in 2**np.arange(8, 14):
            encoder_blocks = blockgen(steps=6, spatial_in=spatial_dim, spatial_out=8, filters_in=2, filters_out=64, mode='encoder')
            decoder_blocks = blockgen(steps=6, spatial_in=8, spatial_out=spatial_dim, filters_in=64, filters_out=2, mode='decoder')
